<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" import="java.util.Date"%>
<!-- New line below to use the JSP Standard Tag Library -->
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
<%@ page isErrorPage="true" %>  
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<!-- for Bootstrap CSS -->
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
<!-- YOUR own local CSS -->
<!-- <link rel="stylesheet" href="/css/main.css"/> -->
<!-- For any Bootstrap that uses JS or jQuery-->
<!-- <script src="/webjars/jquery/jquery.min.js"></script> -->
<!-- <script src="/webjars/bootstrap/js/bootstrap.min.js"></script> -->
<title>Read Share</title>
</head>
<body style="margin:5vw;">


	<div class="container">
		<div class="row justify-content-between">
			<div class="col-4">
				<div class="mb-3">
					<h2>"${book.title}"</h2>	
				</div>
			</div>
			<div class="col-3">
				<div class="mb-3">
					<p><a href="/books">Back to the shelves</a></p>
				</div>
			</div>
		</div>
		<hr />
		<c:choose>
			<c:when test="${book.creator.id == user_id}">
				<h5>You read "${book.title}" by ${book.author}.</h5>
			</c:when>
			<c:otherwise>
				<h5>${book.creator.userName} read "${book.title}" by ${book.author}.</h5>
			</c:otherwise>
		</c:choose>
		<br />
		<c:choose>
			<c:when test="${book.creator.id == user_id}">
				<h5>Here are your thoughts:</h5>
			</c:when>
			<c:otherwise>
				<h5>Here are ${book.creator.userName}'s thoughts:</h5>
			</c:otherwise>
		</c:choose>
		<hr />
		<br />
		<p>${book.thoughts}</p>
		<br />
		<hr />
		<div class="row justify-content-between">
			<c:if test="${book.creator.id == user_id}">
				<div class="col-4">
					<div class="mb-3">
						<button><a href="/books/${book.id}/edit" class="text-warning">Edit</a></button>
					</div>
				</div>
				<div class="col-3">
					<div class="mb-3">
						<form action="/books/${book.id}/delete" method="post">
						<input type="hidden" name="_method" value="delete">
					    	<button class="text-danger">Delete</button>
					    </form>
					</div>
				</div>
			</c:if>
		</div>
	</div>

</body>
</html>